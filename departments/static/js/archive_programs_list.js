// archive_programs_list.js – النسخة النهائية مع تصحيح الجدول
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ========== كلاس فلترة الأقسام (مخصص للأرشيف) ==========
    class ArchiveDepartmentsFilter {
        constructor() {
            this.filters = {
                programTypes: [],
                format: 'all',
        ...
        handleProgramTypeSelection(value, checkbox) {
            if (value === 'all') {
                ...
                this.filters.programTypes = !allChecked ? (typesJson ? JSON.parse(typesJson.textContent) : []) : [];
            } else {
        ...
        showNoResultsMessage(visibleCount) {
            const departmentsList = document.getElementById('archiveDepartmentsList');
            const existingNoDepts = departmentsList.querySelector('.no-programs');
            if (visibleCount === 0) {
                if (!existingNoDepts) {
                    const noDeptsHTML = `
                        <div class="col-12">
    ...
    if (document.querySelector('.archive-programs-page')) {
        new ArchiveDepartmentsFilter();
    }

    // ========== كود الكاروسيل الجديد "Интересные программы" ==========
    const interestingCarousel = document.getElementById('interestingCarousel');
    const carouselDots = document.getElementById('interestingCarouselDots');
    const prevBtn = document.getElementById('interestingPrev');
    const nextBtn = document.getElementById('interestingNext');

    if (interestingCarousel && carouselDots && prevBtn && nextBtn) {
        const slides = interestingCarousel.querySelectorAll('.carousel-slide');
        ...
        function updateCarouselPosition() {
            ...
            interestingCarousel.style.transition = 'transform 0.3s ease';
            interestingCarousel.style.transform = `translateX(-${currentSlide * slideWidth}px)`;
        }
        ...
        interestingCarousel.addEventListener('touchstart', (e) => {
            isDragging = true;
            startPos = e.touches[0].clientX;
            currentTranslate = prevTranslate;
            interestingCarousel.style.transition = 'none';
            stopAutoSlide();
        }, { passive: false });

        interestingCarousel.addEventListener('touchmove', (e) => {
            if (!isDragging) return;
            e.preventDefault();
            const diff = e.touches[0].clientX - startPos;
            interestingCarousel.style.transform = `translateX(${currentTranslate + diff}px)`;
        }, { passive: false });

        interestingCarousel.addEventListener('touchend', (e) => {
            isDragging = false;
            interestingCarousel.style.transition = 'transform 0.3s ease';
        ...
        interestingCarousel.addEventListener('mouseenter', stopAutoSlide);
        interestingCarousel.addEventListener('mouseleave', startAutoSlide);
